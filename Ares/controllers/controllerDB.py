...
class ControllerDB:
	...
	def exerciceExists(self, sExercice):
		return self.DB.exists(QueryTables['ExercicesInstalled'][0], 'Prefix', sExercice)
	...
	def registreTraining(self, sUser, dFields):
		...
		if (self.verifyDictionnary(dFields, True, QueryTables['Trainings'][0])):
			...
			return
	def getExercicesInstalled(self):
		return sum(self.DB.query(""" SELECT Prefix FROM """ + QueryTables['ExercicesInstalled'][0]), () )
	def installExercice(self, dFields):
		if (self.verifyDictionnary(dFields, True, QueryTables['ExercicesInstalled'][0])):
			if(self.DB.exists(QueryTables['ExercicesInstalled'][0], 'Prefix', dFields['Prefix'])):
				#error
				print("ERROR MODULE ALREADY INSTALLED")
				return
			else:
				self.DB.insert(QueryTables['ExercicesInstalled'][0], dFields)
				print('INSERTED!')
		else: 
			return 
	def desinstallExercice(self, sField):
		if(self.exerciceExists(sField)):
			self.DB.delete(QueryTables['ExercicesInstalled'][0], {'Prefix' : sField})
		else:
			#error
			print('ERROR EXERCICE DONT EXISTS')
			return
